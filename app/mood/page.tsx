...
const MOOD_DEFS = [
  { key: "mood_great",      emoji: "😄", color: "#7EDC8A", bg: "#F0FFF4",  burnout: false },
  { key: "mood_good",       emoji: "🙂", color: "#5EA9FF", bg: "#EBF5FF",  burnout: false },
  { key: "mood_okay",       emoji: "😐", color: "#FFD45E", bg: "#FFFDF0",  burnout: false },
  { key: "mood_tired",      emoji: "😴", color: "#8EC5FF", bg: "#EBF5FF",  burnout: true  },
  { key: "mood_stressed",   emoji: "😰", color: "#FFAD5E", bg: "#FFF5EB",  burnout: true  },
  { key: "mood_overwhelmed",emoji: "😫", color: "#FF7B7B", bg: "#FFF5F5",  burnout: true  },
  { key: "mood_sad",        emoji: "😢", color: "#A78BFA", bg: "#F5F0FF",  burnout: true  },
  { key: "mood_anxious",    emoji: "😬", color: "#FFD45E", bg: "#FFFDF0",  burnout: true  },
  { key: "mood_meh",        emoji: "😑", color: "#6B7A99", bg: "#F5FAFF",  burnout: false },
] as const;
...
export default function MoodPage() {
  ...
  const MOODS = MOOD_DEFS.map((m) => ({ ...m, label: t(m.key) }));

  const todayKey = new Date().toISOString().slice(0, 10);
  const alreadyCheckedIn = state.lastMoodDate === todayKey;
  const isBurnout = MOOD_DEFS.find((m) => t(m.key) === selected)?.burnout ?? false;
        ...
        <div style={{ padding: "40px 20px", textAlign: "center" }}>
          ...
          {latestMood && (
            <div className="fv-card" style={{ marginBottom: 20, textAlign: "left" }}>
              <p style={{ margin: "0 0 6px", fontWeight: 800, fontSize: "0.85rem", color: "#1D2B53" }}>{t("todays_mood")}</p>
      ...
      <FVShell>
        <div style={{ padding: "40px 20px", textAlign: "center" }}>
          ...
          <div style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "#FFF8E7", border: "1px solid #FFE08A", borderRadius: 999, padding: "10px 20px", marginBottom: 24 }}>
            ...
            <span style={{ fontWeight: 600, color: "#6B7A99", fontSize: "0.85rem" }}>{t("reward_badge")}</span>
          </div>
          <div className="fv-card" style={{ marginBottom: 16, textAlign: "left" }}>
            <div className="row gap-10">
              <Sparkles size={18} color="#5EA9FF" />
              <p style={{ margin: 0, fontWeight: 800, fontSize: "0.85rem", color: "#1D2B53" }}>{t("ai_suggestion")}</p>
            </div>
        ...
        <div style={{ padding: "24px 20px" }}>
          ...
          <div style={{ textAlign: "center", marginBottom: 16 }}>
            <div style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "#FFF8E7", border: "1px solid #FFE08A", borderRadius: 999, padding: "8px 16px" }}>
              ...
              <span style={{ color: "#6B7A99", fontSize: "0.8rem", fontWeight: 600 }}>{t("reward_badge")}</span>
            </div>
          </div>
